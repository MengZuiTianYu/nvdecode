...
/* 启动解码 */
int RtspProcess::Decode(std::string filepath, const int gpuid, int savePath)
{
    typedef struct
    {
    ...
    int64_t timstamp = 0;
    /* 解码是否启用时间戳的标志位 */
    uint32_t flags = CUVID_PKT_TIMESTAMP;
    /* 解码后得到的显示时间戳变量 */
    int64_t *ppTimestamp;
    ck(cuInit(0));
    int nGpu = 0;
    ...
    if(avformat_open_input(&ctx, filepath.c_str(), NULL, NULL) != 0)
    {
        printf("\ncamera device ip is error!\n");
        return 0;
    }
    else
    {
    ...
    do
    {
        demuxer->Demux(&pVideo, &nVideoBytes, &timstamp);//加入了时间戳
        std::cout << "read stamp:" << timstamp << std::endl;
        dec->Decode(pVideo, nVideoBytes, &ppFrame, &nFrameReturned, CUVID_PKT_TIMESTAMP, &ppTimestamp, timstamp);
        ...
        for (int i = 0; i < nFrameReturned; i++)
        {
            ...
            // ptr << "IMAGE" << savePath;
            //std::string savefilepath = "/home/zhangl" + ptr.str() + "/image" + str.str();
            //LOG(INFO)<<"After string" << savefilepath;
            //cv::imwrite("/home/zhangl/cameraImage/" + ptr.str() + "/image" + str.str(), g_img);
            //cv::imshow("test", c_img);
            //cv::waitKey(1);
            temp++;
            std::cout << "the display stamp is:" << ppTimestamp[i] << std::endl;
        }
        //printf("time: %fms\n", time);
...
void RtspProcess::startThread(std::string rtsppath, const int gpuID, int saveCountPath)
{
