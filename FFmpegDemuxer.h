...
private:
    FFmpegDemuxer(AVFormatContext *fmtc) : fmtc(fmtc) {
        if (!fmtc) {
//            std::cout << "No AVFormatContext provided.";
            std::cout << "No AVFormatContext provided.";
            return;
        }
...
//        LOG(INFO) << "Media format: " << fmtc->iformat->long_name << " (" << fmtc->iformat->name << ")";
        ...
        if (iVideoStream < 0) {
          std::cout << "FFmpeg error: " << __FILE__ << " " << __LINE__ << " " << "Could not find stream in input file";
//          std::cout << "FFmpeg error: " << __FILE__ << " " << __LINE__ << " " << "Could not find stream in input file";
            return;
        }
...
public:
    ...
    bool Demux(uint8_t **ppVideo, int *pnVideoBytes, int64_t *timestamp) {
        if (!fmtc) {
            return false;
        }
        ...
        *pnVideoBytes = 0;
        ...
        if (bMp4H264 || bMp4HEVC) {
            ...
            /* 若是视频文件，则进这里 */
            *timestamp = pktFiltered.pts*(av_q2d(fmtc->streams[pktFiltered.stream_index]->time_base)*1000);
            *ppVideo = pktFiltered.data;
            *pnVideoBytes = pktFiltered.size;
        } else {
            /* 若是rtsp流，则进这里 */
            *timestamp = pkt.pts*(av_q2d(fmtc->streams[pkt.stream_index]->time_base)*1000);
            *ppVideo = pkt.data;
